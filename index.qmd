...
```{python}
from step2_create_stipple import create_stipple
stipple_pattern, _ = create_stipple(gray_image, percentage=0.08)
```

# Step 3 
```{python}
from step3_create_tonal import create_tonal
tonal_image, average_tones, tonal_stats = create_tonal(gray_image, 16, 12, return_full_image=True)
```
...
def create_block_letter_s(height, width, letter="S", font_size_ratio=0.9):
    """
    Creates a grayscale block letter mask (black letter on white background).
    """
...
# Now create the block letter mask
...
def create_masked_stipple(stipple_img, mask_img, threshold=0.5):
    """
    Apply a block letter mask to the stippled image.
    Pixels under the threshold in the mask are removed (set to white).
    """
    masked = np.where(mask_img < threshold, 1.0, stipple_img)
    return masked
...
# Display masked stippled image
import matplotlib.pyplot as plt
fig, ax = plt.subplots(figsize=(6.5, 5))
ax.imshow(masked_stipple, cmap='gray', vmin=0, vmax=1)
ax.axis('off')
ax.set_title('Step 5: Masked Stipple (Selection Bias Applied)', fontsize=14, fontweight='bold', pad=10)
plt.tight_layout()
plt.show()
...
def create_statistics_meme(original_img, stipple_img, block_letter_img, masked_stipple_img,
                           output_path, dpi=150, background_color="white"):
    """
    Assemble all panels into a four-panel statistics meme.
    """
    fig, axes = plt.subplots(1, 4, figsize=(8, 3), dpi=dpi)
    fig.patch.set_facecolor(background_color)
