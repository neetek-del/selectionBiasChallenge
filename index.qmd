...
img = Image.open("ImageNeetekKumar.jpg").convert("L")
gray_image = np.array(img).astype(float) / 255.0
```
...
stipple_pattern = uniform_filter(gray_image, size=5)
stipple_pattern = np.where(stipple_pattern > 0.5, 1.0, 0.0)
fig, ax = plt.subplots(figsize=(6.5, 5))
ax.imshow(stipple_pattern, cmap='gray', vmin=0, vmax=1)
ax.axis('off')
ax.set_title('Step 2: Stippled Image', fontsize=14, fontweight='bold', pad=10)
...
# Step 3 — Load the block-letter and masking functions
...
# Step 4 —Create Block Letter Mask

```{python}
from step4_create_block_letter import create_block_letter_s

# Generate the block letter S mask
h, w = gray_image.shape
...
# Step 5 — Apply Mask to Stippled Image
```{python}
from step5_create_masked import create_masked_stipple

# Apply mask to remove stipple inside the S
masked_stipple = create_masked_stipple(
    stipple_img=stipple_pattern,
...
# Step 6 — Create the Final Statistics Meme
```{python}
from create_meme import create_statistics_meme
