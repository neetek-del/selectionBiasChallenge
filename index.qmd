...
** FILE 1 — step4_create_block_letter.py (unchanged from starter file which is the same as the one in the repository)
...
** FILE 2 — step5_create_masked.py (unchanged from starter file which is the same as the one in the repository)
...
# Import function or define inline
try:
    from step4_create_block_letter import create_block_letter_s
except ImportError:
    import numpy as np
    ...
    def create_block_letter_s(height: int,
        ...
        """
        Creates a grayscale block letter mask (black letter on white background)
        representing systematic missing data.
        """
        img = Image.new("L", (width, height), color=255)
        draw = ImageDraw.Draw(img)
...
print("🎨 Step 4: Creating Selection Bias Pattern")
print("=" * 50)
...
# Statistics
mask_pixels = np.sum(block_letter < 0.5)
...
print(f"📊 Block Letter Mask Statistics:")
...
print()
print("⚠️  This 'S' region represents the systematic missing data pattern.")
print(f"   Applying it will remove {mask_percentage:.2f}% of the image.")
print("=" * 50)
...
ax.imshow(block_letter, cmap='gray', vmin=0, vmax=1)
ax.axis('off')
ax.set_title('Step 4: Block Letter Mask (S)', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.show()
...
# Import or define masking function
try:
    from step5_create_masked import create_masked_stipple
except ImportError:
    def create_masked_stipple(stipple_img: np.ndarray,
                              ...
                              threshold: float = 0.5) -> np.ndarray:
        """
        Applies mask: where mask < threshold → remove stipples (set to 1.0).
        """
        return np.where(mask_img < threshold, 1.0, stipple_img)


print("🔍 Step 5: Applying Selection Bias Mask")
print("=" * 50)

# --- BEFORE MASKING ---
stipple_pixels = np.sum(stipple_pattern < 0.5)
total_pixels = stipple_pattern.size
stipple_pct = stipple_pixels / total_pixels * 100

print("📊 Stippled Image Statistics:")
print(f"   Total pixels: {total_pixels:,}")
print(f"   Stipple points (black pixels): {stipple_pixels:,}")
print(f"   Stipple coverage: {stipple_pct:.2f}%")
print()

# --- NEW TONAL STATISTICS BLOCK ---
mean_brightness = np.mean(stipple_pattern)
std_brightness = np.std(stipple_pattern)
min_brightness = np.min(stipple_pattern)
max_brightness = np.max(stipple_pattern)

print("📊 Tonal Statistics for Parameter Tuning:")
print(f"  Mean brightness: {mean_brightness:.3f}")
print(f"  Standard deviation: {std_brightness:.3f}")
print(f"  Brightness range: [{min_brightness:.3f}, {max_brightness:.3f}]")
print()
print("💡 Tuning Tips:")
print("  - If mean < 0.4: Image is dark → lower extreme_threshold_low")
print("  - If mean > 0.6: Image is light → raise extreme_threshold_high")
print("  - If std > 0.2: High contrast → stronger extreme_downweight may help")
print("  - mid_tone_center ≈ 0.28 emphasizes the typical/average tones")
print()
...
# --- AFTER MASKING ---
masked_pixels = np.sum(masked_stipple < 0.5)
masked_pct = masked_pixels / total_pixels * 100
removed_pixels = stipple_pixels - masked_pixels
removed_pct = (removed_pixels / stipple_pixels) * 100

print("📊 Masked Image Statistics:")
print(f"   Remaining stipple points: {masked_pixels:,}")
print(f"   Remaining coverage: {masked_pct:.2f}%")
print(f"   Removed stipple points: {removed_pixels:,}")
print(f"   Data loss: {removed_pct:.2f}%")
print()
print(f"⚠️  Selection bias removed {removed_pct:.1f}% of the stipple data!")
print("=" * 50)
...
ax.imshow(masked_stipple, cmap='gray', vmin=0, vmax=1)
ax.axis('off')
ax.set_title('Step 5: Masked Stippled Image (Estimate)', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.show()
```
...
def create_statistics_meme(
    ...
    fig, axes = plt.subplots(1, 4, figsize=(8, 4), dpi=dpi)
    fig.patch.set_facecolor(background_color)
